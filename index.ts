...
import updateStates from "./utils/updateState";
import { clone, equals, isNullOrEmpty } from "./utils/utils";
...
export default class UpdateStateComponent<T, K> extends React.Component<T, K> {
  ...
  public static updateStateFromProps(propsData, preState) {
    let data = null;
    ...
    Object.keys(propsData).forEach((key) => {
      ...
      if (!equals(originData, newPropData)) {
        data = {
          ...data,
          ...{ [key]: newPropData, [`origin${key}`]: clone(newPropData) },
        };
      }
    });
    return data;
